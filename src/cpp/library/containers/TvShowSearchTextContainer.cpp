...
namespace Library::Containers {
    ...
    TvShowSearchTextContainer::TvShowSearchTextContainer(std::string p_search_text, int p_season, bool auto_load, bool* p_found) {
        if (auto_load){
    ...
    TvShowSearchTextContainer::TvShowSearchTextContainer(std::string p_search_text, std::string p_show_title, std::string p_show_date) {
    ...
    bool TvShowSearchTextContainer::reloadFromDb() {
        sqlite3_stmt* stmt = Global::library_db->simpleStatementFromString("SELECT show_title, show_date, FROM tv_show_search_text WHERE search_text = ?;");
    ...
    bool TvShowSearchTextContainer::writeToDb() {
        if (existsInDb()) {
            sqlite3_stmt* stmt = Global::library_db->simpleStatementFromString(
                "UPDATE tv_show_search_text SET show_title = ?, show_date = ? WHERE search_text = ?;"
            );
            ...
            sqlite3_bind_text(stmt, 3, search_text.c_str(), search_text.length(), NULL);
            ...
            sqlite3_stmt* stmt = Global::library_db->simpleStatementFromString(
                "INSERT INTO tv_season (search_text, show_title, show_date) VALUES (?, ?, ?);"
            );
            sqlite3_bind_text(stmt, 1, search_text.c_str(), search_text.length(), NULL);
...
#ifdef UI_BUILD
    void TvShowSearchTextContainer::loadPosterImg() {
