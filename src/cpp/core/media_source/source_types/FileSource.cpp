...
void FileSource::scanForMovies(std::stop_token stoken,
    ...
    scanDirectoryForMovies(stoken, src_name, fs::path(get_rootPath()), &selected_files);
    ...
    for (std::list<fs::path>::iterator it = selected_files.begin(); it != selected_files.end(); ++it) {
        ...
        // Updating scraping progress:
        {
            ...
            PLOG_DEBUG.printf("Progress: %f, Scraping '%s'", _scanProgress, _currentScanPath.c_str());
        }
...
void FileSource::scanDirectoryForMovies(std::stop_token stoken,
    ...
    for (const fs::directory_entry& entry : fs::directory_iterator(dir)) {
        ...
        if (entry.is_directory()) {
            ...
            scanDirectoryForMovies(stoken, src_name, entry, p_selected_files);
            ...
            p_selected_files->push_back(rel_path);
        }
    }
}
